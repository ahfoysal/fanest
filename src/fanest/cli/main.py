...
def generate_resource(name: str) -> None:
    resource = _ensure_resource_dir(name, exist_ok=False)
    class_name = _class_name(name)
    (resource / f"{name}_service.py").write_text(_service_template(class_name), encoding="utf-8")
    (resource / f"{name}_controller.py").write_text(
        _controller_template(name, class_name), encoding="utf-8"
    )
    (resource / f"{name}_module.py").write_text(_module_template(name, class_name), encoding="utf-8")
    typer.echo(f"Generated resource {name}")


@generate_app.command("module")
def generate_module(name: str) -> None:
    resource = _ensure_resource_dir(name)
    class_name = _class_name(name)
    (resource / f"{name}_module.py").write_text(_module_template(name, class_name), encoding="utf-8")
...
def generate_guard(name: str) -> None:
    resource = _ensure_resource_dir(name)
    class_name = _class_name(name)
    (resource / f"{name}_guard.py").write_text(_guard_template(class_name), encoding="utf-8")
    typer.echo(f"Generated guard {name}")


@generate_app.command("pipe")
def generate_pipe(name: str) -> None:
    resource = _ensure_resource_dir(name)
    class_name = _class_name(name)
    (resource / f"{name}_pipe.py").write_text(_pipe_template(class_name), encoding="utf-8")
    typer.echo(f"Generated pipe {name}")


@generate_app.command("interceptor")
def generate_interceptor(name: str) -> None:
    resource = _ensure_resource_dir(name)
    class_name = _class_name(name)
    (resource / f"{name}_interceptor.py").write_text(
        _interceptor_template(class_name), encoding="utf-8"
    )
    typer.echo(f"Generated interceptor {name}")


@generate_app.command("filter")
def generate_filter(name: str) -> None:
    resource = _ensure_resource_dir(name)
    class_name = _class_name(name)
    (resource / f"{name}_filter.py").write_text(_filter_template(class_name), encoding="utf-8")
    typer.echo(f"Generated filter {name}")


@generate_app.command("gateway")
def generate_gateway(name: str) -> None:
    resource = _ensure_resource_dir(name)
    class_name = _class_name(name)
    (resource / f"{name}_gateway.py").write_text(_gateway_template(name, class_name), encoding="utf-8")
